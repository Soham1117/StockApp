...
class Configuration:
    ...
    def get_duckdb_settings(self):
        # Skip cache_httpfs when using local parquet files (avoids needing
        # the community extension, which isn't available on all platforms)
        use_local = bool(os.getenv("DEFEATBETA_LOCAL_DATA"))
